...
module IterMonad = {
  ...
  let return = x => [@implicit_arity] Return(x, empty_stream);
  let fail = raise;
  let rec bind = (v, f) =>
    switch (v) {
    | [@implicit_arity] Return(x, s) =>
      switch (f(x)) {
      | [@implicit_arity] Return(x, _) => [@implicit_arity] Return(x, s)
      | Continue(k) => k(s)
      }
    | Continue(k) => Continue((stream => bind(k(stream), f)))
    };
  ...
  let rec get =
    Continue(
      fun
      | EOF => [@implicit_arity] Return(None, EOF)
      | Chunk(s) =>
        if (s.i < s.len) {
          [@implicit_arity]
          Return(Some(Bytes.get(s.buf, s.i)), Chunk({...s, i: s.i + 1}));
        } else {
          get;
        },
    );
...
let parse_document = inc => {
  ...
  and loop = (v, inp) =>
    switch (v) {
    | [@implicit_arity] Return(Some(result), rest) =>
      loop(process_token(result), rest)
    | [@implicit_arity] Return(None, rest) => ()
    | Continue(cont) =>
      let rec upload = (f, rest) => {
        ...
        switch (f(stream)) {
        | [@implicit_arity] Return(None, rest) => ()
        | [@implicit_arity] Return(Some(r), rest) =>
          loop(process_token(r), rest)
